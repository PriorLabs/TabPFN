...
in `ENABLED_PLATFORMS` share a single reference set. The tests are skipped on other
platforms. If a platform turns out to diverge, reintroduce a per-platform reference
directory for it (see the git history of this file for how that looked).
...
set. This covers all CI runners (macos, ubuntu, windows). All enabled platforms are
expected to reproduce the same shared float64 reference predictions."""
...
def _get_reference_path(test_name: str) -> pathlib.Path:
    return REFERENCE_DIR / f"{test_name}.json"
...
def save_reference_predictions() -> None:
    """Generate and save the reference predictions."""

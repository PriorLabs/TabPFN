...
def test_scale_n_estimators_for_feature_coverage__default_from_checkpoint_is_used():
    """`"auto"` starts from the count the checkpoint declares, not the constant."""
    cfg = PreprocessorConfig("none", max_features_per_estimator=500)
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        result = scale_n_estimators_for_feature_coverage(
...
def test_scale_n_estimators_for_feature_coverage__checkpoint_default_still_scales():
    """A checkpoint's count is a base value, not a ceiling.

    Coverage scaling still raises it on a table too wide for that many estimators to
    see every feature.
    """
    cfg = PreprocessorConfig("none", max_features_per_estimator=500)
    with pytest.warns(UserWarning, match="Auto-scaling n_estimators from 3 to 5"):
        result = scale_n_estimators_for_feature_coverage(
            n_estimators="auto",
            n_total_features=2500,
            preprocessor_configs=[cfg],
            default_n_estimators=3,
        )
    assert result == 5
...
def test_scale_n_estimators_for_feature_coverage__explicit_beats_checkpoint_default():
    """An explicit count wins over the checkpoint's and is never scaled."""
    cfg = PreprocessorConfig("none", max_features_per_estimator=500)
    with warnings.catch_warnings():
        warnings.simplefilter("error")
        result = scale_n_estimators_for_feature_coverage(

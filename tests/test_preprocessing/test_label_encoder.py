...
class TestTabPFNLabelEncoderBasic:
    ...
    def test__fit_transform__string_labels(self) -> None:
        ...
        y_encoded, metadata = encoder.fit_transform(y, max_num_classes=10)

        assert y_encoded.shape == (5,)
        assert metadata.n_classes == 3
        np.testing.assert_array_equal(metadata.classes, ["bird", "cat", "dog"])
        np.testing.assert_array_equal(metadata.class_counts, [1, 2, 2])
    ...
    def test__fit_transform__non_contiguous_integers(self) -> None:
        ...
        assert metadata.n_classes == 3
        np.testing.assert_array_equal(metadata.classes, [0, 5, 10])
        np.testing.assert_array_equal(metadata.class_counts, [2, 2, 3])
    ...
    def test__fit_transform__negative_integers(self) -> None:
        ...
        np.testing.assert_array_equal(metadata.classes, [-1, 0, 1])
...
class TestTabPFNLabelEncoderUnconventionalInputs:
    ...
    def test__fit_transform__boolean_labels(self) -> None:
        ...
        _, metadata = encoder.fit_transform(y, max_num_classes=10)

        assert metadata.n_classes == 2
        # Booleans sort as False < True
        np.testing.assert_array_equal(metadata.class_counts, [2, 3])
    ...
    def test__fit_transform__pandas_series(self) -> None:
        ...
        assert metadata.n_classes == 3
        np.testing.assert_array_equal(metadata.classes, ["a", "b", "c"])
    ...
    def test__fit_transform__unicode_strings(self) -> None:
        ...
        assert metadata.n_classes == 3
        assert "café" in metadata.classes

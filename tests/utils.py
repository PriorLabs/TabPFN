...
def mark_mps_configs_as_non_ci(configs: Iterable[tuple]) -> Generator[tuple]:
    """Add a pytest "non_ci" mark to any configurations that run on MPS.
    ...
    for config in configs:
        if config[0] == "mps":
            yield pytest.param(*config, marks=pytest.mark.non_ci)
        else:
            yield config

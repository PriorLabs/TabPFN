...
A: Recommended row and feature limits vary by checkpoint — see the [Models page](https://docs.priorlabs.ai/models) for the per-release limits. As a quick reference: **TabPFN-3** is recommended for up to **1,000,000 rows** and **2,000 features**, and **TabPFN-2.6** for up to **100,000 rows** and **2,000 features**. If your dataset exceeds the recommended row limit for your checkpoint, you can subsample, set `ignore_pretraining_limits=True` to push past the size guardrail, or upgrade to a release with a higher limit.

...
It is also missing the KV cache, and so is not suitable for public deployment.
...
from tabpfn.architectures.base import EncoderType  # noqa: TC001
...
class TabPFNV2p5Config(ArchitectureConfig):
    ...
    """Determines (among potentially other things) to encode a treatment variable."""
...
class AlongColumnAttention(Attention):
    ...
    def forward(
        ...
        output_BcSHD = self._attend_local(q_BcRHD, k_BcNHD, v_BcNHD, N, R)
    ...
    def _attend_local(
        self,
        q_BcRHD: torch.Tensor,
        k_BcNHD: torch.Tensor,
        v_BcNHD: torch.Tensor,
        N: int,
        R: int,
    ) -> torch.Tensor:
        """Attention between train and test rows."""
        if N == R:
            return _batched_scaled_dot_product_attention(q_BcRHD, k_BcNHD, v_BcNHD)
        ...
        return torch.cat([out_train_BcNHD, out_test_BcMHD], dim=1)
...
class TabPFNV2p5(Architecture):
    ...
    def __init__(
        ...
        *,
        ...
        self.encode_treatment = config.interventions_enabled
    ...
    def _run_blocks(
        self,
        x_BRCD: torch.Tensor,
        num_train_and_thinking_rows: int,
        *,
        save_peak_memory_factor: int | None,
        ...
        return x_BRCD, num_train_and_thinking_rows
    ...
    def forward(
        ...
        *,
        ...
        x_BRCD, num_train_and_thinking_rows = self._run_blocks(
            x_BRCD,
            num_train_and_thinking_rows,
    ...
    def _preprocess_and_embed_features(
        ...
        if not self.encode_treatment:
            nan_and_inf_indicator_RiBgF = _generate_nan_and_inf_indicator(x=x_RiBgF)
        else:
            nan_and_inf_indicator_RiBgF = _generate_nan_and_treatment_indicator(
                x=x_RiBgF
            )
...
def _generate_nan_and_treatment_indicator(x: torch.Tensor) -> torch.Tensor:
    """Generate the treatment indicators for the first feature group.

    Re-uses/purposes encodings learned by TabPFN-2.5 regression checkpoints
    to encode the first feature group as the treatment variable, which is
    a structural variable in the causal inference task.

    Returns:
        A tensor with the shape (Ri, G, F), where
            - Ri = number of input rows (train + test)
            - G = number of feature groups
            - F = number of features/group
    """
    # flags for the treatment and control group
    t_1 = x[:, 0, 0] == 1
    t_0 = x[:, 0, 0] == 0

    # still use the nan indicator to flag nans
    nan_and_treatment_indicator = (torch.isnan(x) * NAN_INDICATOR).to(x.dtype)

    # encodes the first feature group as the treatment variable
    # pos. inf encodes treatment, neg. inf. encodes control
    nan_and_treatment_indicator[:, 0, :][t_1] = INFINITY_INDICATOR
    nan_and_treatment_indicator[:, 0, :][t_0] = NEG_INFINITY_INDICATOR

    return nan_and_treatment_indicator

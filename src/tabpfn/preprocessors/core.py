"""Core preprocessing orchestration and helpers."""
...
def generate_classifier_ensemble_configs(  # noqa: PLR0913
    *,
    num_estimators: int,
    subsample_samples: int | float | list[np.ndarray] | None,
    max_index: int,
    ...
    if class_shift_method == "rotate":
        ...
        class_permutations = [None] * num_estimators
    else:
        raise ValueError(f"Invalid {class_shift_method=}")
    ...
    if leftover > 0:
        configs_ += preprocessor_configs[:leftover]

    model_indices = [i % num_models for i in range(num_estimators)]

    return [
        ClassifierEnsembleConfig(
            preprocess_config=prep_conf,
        ...
        for featshift, class_perm, subsample_ix, prep_conf, model_index in zip(
            featshifts,
            class_permutations,
            subsample_indices,
            configs_,
...
def generate_regressor_ensemble_configs(  # noqa: PLR0913
    *,
    num_estimators: int,
    subsample_samples: int | float | list[np.ndarray] | None,
    max_index: int,
...
def fit_preprocessing_one(
    ...
    *,
    ...
    preprocessor = build_preprocessing_pipeline(config, random_state=static_seed)
...
class DatasetCollectionWithPreprocessing(Dataset):
    ...
    def __getitem__(self, index: int):  # noqa: ANN204
        """Retrieve, split, fit, and transform a dataset configuration."""

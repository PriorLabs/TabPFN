...
def is_gpu_oom_error(error: BaseException) -> bool:
    """Check if an error is a CUDA or MPS out-of-memory error.

    Args:
        error: The exception to check.

    Returns:
        True if the error is a GPU memory error, False otherwise.
    """
    ...
    is_cuda_oom = "cuda" in error_msg and (
        "out of memory" in error_msg or "invalid configuration" in error_msg
    )
    is_mps_oom = "mps" in error_msg and "out of memory" in error_msg

    return is_cuda_oom or is_mps_oom


def handle_prediction_memory_error(
    error: RuntimeError,
    X: XType,
    model_type: Literal["classifier", "regressor"],
) -> None:
    """Handle CUDA/MPS out-of-memory errors during prediction.

    Raises a more helpful RuntimeError with guidance on how to batch predictions.
    Should only be called after checking is_gpu_oom_error(error) returns True.

    Args:
        error: The original RuntimeError from the forward pass.
        X: The input data that caused the error.
        model_type: Type of model ("classifier" or "regressor") for error message.

    Raises:
        RuntimeError: A more informative error with batching instructions.
    """
    error_msg = str(error).lower()
    is_mps_oom = "mps" in error_msg and "out of memory" in error_msg

    n_samples = X.shape[0] if hasattr(X, "shape") else len(X)
    device_type = "MPS" if is_mps_oom else "CUDA"
    ...
    raise RuntimeError(
        f"{device_type} out of memory during prediction with "
        f"{n_samples} samples.\n\n"
        f"Please chunk your test set into smaller batches:\n\n"
        f"    batch_size = 1000\n"
        f"    predictions = []\n"
        f"    for i in range(0, len(X_test), batch_size):\n"
        f"        batch = X_test[i:i+batch_size]\n"
        f"        predictions.append(model.{predict_method}(batch))\n"
        f"    predictions = np.vstack(predictions)\n\n"
        f"Alternative solutions:\n"
        f"  - Use CPU inference: {model_class}(device='cpu')\n"
        f"  - Reduce the number of test samples\n"
        f"  - Use a GPU with more memory\n\n"

...
class ModelConfig:
    ...
    item_attention_type: Literal["full", "linear"] = "full"
    """Specifies the attention method to use for between items.
        full: standard quadratic attention, see model.attention.full_attention
        linear: linear attention, see model.attention.linear_attention
    """
    feature_attention_type: Literal["full", "linear"] = "full"
    """Specifies the attention method to use for between features."""
    linear_attention_impl: Literal["einsum", "bmm"] = "bmm"
    """The implementation of linear attention to use."""
    linear_attention_feature_map: Literal["elu"] = "elu"
    linear_attention_norm_q: bool = True
    """If True and attention_type is "linear", then the queries are normalized after the
    feature map"""
    linear_attention_norm_k: bool = True
    """If True and attention_type is "linear", then the keys are normalized after the
    feature map"""
    linear_attention_eps: float = 1e-6
    """If True and attention_type is "linear", used to avoid division by zero."""
    seed: int = 0
